...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    if (type == 1) type_str = "tree";
    else if (type == 2) type_str = "commit";
    ...
    memcpy(full_buffer, header, header_len + 1);
    memcpy(full_buffer + header_len + 1, data, len);

    // Step 2: Compute SHA-256 hash of the FULL object (header + data)
    compute_hash(full_buffer, full_size, id_out);

    // Step 3: Check if object already exists (deduplication) — if so, just return success
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    if (!file_buffer) { fclose(f); return -1; }

    if (fread(file_buffer, 1, file_size, f) != (size_t)file_size) {
        free(file_buffer); fclose(f); return -1;
    }
    fclose(f);

    // Step 4: Verify integrity: recompute the SHA-256 of the file contents
    // and compare to the expected hash (from *id). Return -1 if mismatch.
    ...
    if (!null_pos) { free(file_buffer); return -1; }
    size_t header_len = null_pos - file_buffer;
    char type_str[16];
    size_t data_size;
    if (sscanf((char *)file_buffer, "%15s %zu", type_str, &data_size) != 2) {
        free(file_buffer); return -1;
    }
    // Step 5: Set *type_out to the parsed ObjectType
    if (strcmp(type_str, "blob") == 0) *type_out = OBJ
    ...
    if (header_len + 1 + data_size != (size_t)file_size)
    {
        free(file_buffer); return -1;
    }
    void *data_buffer = malloc(data_size);
    if (!data_buffer) { free(file_buffer); return -1; }
    memcpy(data_buffer, null_pos + 1, data_size);
